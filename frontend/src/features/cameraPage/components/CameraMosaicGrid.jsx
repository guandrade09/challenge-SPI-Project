  ...
  return (
    <div className="w-full flex flex-col gap-1.5 sm:gap-2 mt-2 sm:mt-4">
      <div className="flex items-center justify-between px-1">
        <span className="text-theme-head">
            ...
            <div
              ...
              className={`group relative flex flex-col justify-between p-2 sm:p-2.5 rounded-xl border transition-all duration-200 text-left overflow-hidden shrink-0 w-[140px] sm:w-auto cursor-pointer ${
                isActive
                  ...
                  : 'bg-[var(--p-bg)] border-theme-divider hover:border-[var(--p-border)] hover:bg-[var(--p-header-bg)] opacity-70 hover:opacity-100'
              }`}
            >
              {/* Header do Card Miniatura */}
              <div className="flex items-center justify-between w-full mb-1.5 sm:mb-2 pointer-events-none">
                <span className="text-[11px] sm:text-xs font-bold font-theme-title text-theme-title truncate max-w-[75%]">
                  {cam.nome || `CAM-${idx + 1}`}
                </span>
                {isActive ? (
                  <CheckCircle2 className="w-3.5 h-3.5 text-theme-accent shrink-0" />
                ) : (
                  <Video className="w-3.5 h-3.5 text-theme-muted group-hover:text-theme-title shrink-0 transition-colors" />
                )}
              </div>
              ...
              <div className="w-full h-12 sm:h-16 rounded-lg bg-[var(--p-graf-bg)] border border-theme-divider flex items-center justify-center relative overflow-hidden pointer-events-none">
                ...
                <span className="text-[9px] sm:text-[10px] font-mono text-theme-muted z-20 font-bold uppercase tracking-wider truncate px-1">
                  {cam.setor || 'CCTV'}
                </span>
              </div>

              {/* IP / Identificador no Footer */}
              <span className="text-[8px] sm:text-[9px] font-mono text-theme-muted mt-1 sm:mt-1.5 truncate w-full pointer-events-none">
                {cam.ip || `ID: ${cam.id}`}
              </span>
            </div>
          );
