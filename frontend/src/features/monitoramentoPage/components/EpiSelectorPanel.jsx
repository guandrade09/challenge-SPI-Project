...
export function EpiSelectorPanel({ epis = [], activeEpis = [], onToggleEpi }) {
  return (
    <div className="w-full max-w-4xl mx-auto p-5 rounded-xl border border-theme-divider/30 bg-neutral-950/20 backdrop-blur-sm flex flex-col items-center justify-center space-y-4">
      ...
      <div className="flex flex-wrap gap-2.5 justify-center">
        {epis.map((epi) => {
          // 🚀 CORREÇÃO: Agora verifica se o nome do EPI atual está contido no array de ativos
          const isSelected = activeEpis.includes(epi.nome);
          
          return (
            <button 
              key={epi.id} 
              // 🚀 CORREÇÃO: Aciona o callback passando o nome do item iterado atual
              onClick={() => onToggleEpi(epi.nome)}
              className={`px-4 py-1.5 border rounded-xl text-xs font-mono uppercase tracking-wider shadow-md transition-all transform active:scale-95 h-full ${
                isSelected 
